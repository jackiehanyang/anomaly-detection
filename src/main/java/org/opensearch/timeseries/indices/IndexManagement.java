...
public abstract class IndexManagement<IndexType extends Enum<IndexType> & TimeSeriesIndex> implements LocalNodeClusterManagerListener {
    ...
    public void onClusterManager() {
        try {
            ...
            scheduledRollover = threadPool
                .scheduleWithFixedDelay(() -> rolloverAndDeleteHistoryIndex(), TimeValue.timeValueMinutes(2), executorName());
        } catch (Exception e) {
    ...
    private void handleCustomResultIndex(Config config) {
        ...
        if (config.getCustomResultIndexOrAlias().startsWith(CUSTOM_RESULT_INDEX_PREFIX)) {
            ...
            proceedWithRolloverAndDelete(
                ...
                rolloverRequest,
                getAllCustomResultIndexPattern(config.getCustomResultIndexOrAlias()),
                (IndexType) ForecastIndex.CUSTOM_RESULT,
                config.getCustomResultIndexTTL()
            );
        }
    }
